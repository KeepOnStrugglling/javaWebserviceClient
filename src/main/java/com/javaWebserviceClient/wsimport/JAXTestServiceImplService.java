
package com.javaWebserviceClient.wsimport;

import java.net.MalformedURLException;
import java.net.URL;
import javax.xml.namespace.QName;
import javax.xml.ws.Service;
import javax.xml.ws.WebEndpoint;
import javax.xml.ws.WebServiceClient;
import javax.xml.ws.WebServiceException;
import javax.xml.ws.WebServiceFeature;


/**
 * This class was generated by the JAX-WS RI.
 * JAX-WS RI 2.2.9-b130926.1035
 * Generated source version: 2.2
 * 
 */
@WebServiceClient(name = "JAXTestServiceImplService", targetNamespace = "impl.server.webservice.javatest.com", wsdlLocation = "http://localhost:9010/javatest/webservice/getJAX?wsdl")
public class JAXTestServiceImplService
    extends Service
{

    private final static URL JAXTESTSERVICEIMPLSERVICE_WSDL_LOCATION;
    private final static WebServiceException JAXTESTSERVICEIMPLSERVICE_EXCEPTION;
    private final static QName JAXTESTSERVICEIMPLSERVICE_QNAME = new QName("impl.server.webservice.javatest.com", "JAXTestServiceImplService");

    static {
        URL url = null;
        WebServiceException e = null;
        try {
            url = new URL("http://localhost:9010/javatest/webservice/getJAX?wsdl");
        } catch (MalformedURLException ex) {
            e = new WebServiceException(ex);
        }
        JAXTESTSERVICEIMPLSERVICE_WSDL_LOCATION = url;
        JAXTESTSERVICEIMPLSERVICE_EXCEPTION = e;
    }

    public JAXTestServiceImplService() {
        super(__getWsdlLocation(), JAXTESTSERVICEIMPLSERVICE_QNAME);
    }

    public JAXTestServiceImplService(WebServiceFeature... features) {
        super(__getWsdlLocation(), JAXTESTSERVICEIMPLSERVICE_QNAME, features);
    }

    public JAXTestServiceImplService(URL wsdlLocation) {
        super(wsdlLocation, JAXTESTSERVICEIMPLSERVICE_QNAME);
    }

    public JAXTestServiceImplService(URL wsdlLocation, WebServiceFeature... features) {
        super(wsdlLocation, JAXTESTSERVICEIMPLSERVICE_QNAME, features);
    }

    public JAXTestServiceImplService(URL wsdlLocation, QName serviceName) {
        super(wsdlLocation, serviceName);
    }

    public JAXTestServiceImplService(URL wsdlLocation, QName serviceName, WebServiceFeature... features) {
        super(wsdlLocation, serviceName, features);
    }

    /**
     * 
     * @return
     *     returns JAXTestService
     */
    @WebEndpoint(name = "JAXTestServiceImplPort")
    public JAXTestService getJAXTestServiceImplPort() {
        return super.getPort(new QName("impl.server.webservice.javatest.com", "JAXTestServiceImplPort"), JAXTestService.class);
    }

    /**
     * 
     * @param features
     *     A list of {@link javax.xml.ws.WebServiceFeature} to configure on the proxy.  Supported features not in the <code>features</code> parameter will have their default values.
     * @return
     *     returns JAXTestService
     */
    @WebEndpoint(name = "JAXTestServiceImplPort")
    public JAXTestService getJAXTestServiceImplPort(WebServiceFeature... features) {
        return super.getPort(new QName("impl.server.webservice.javatest.com", "JAXTestServiceImplPort"), JAXTestService.class, features);
    }

    private static URL __getWsdlLocation() {
        if (JAXTESTSERVICEIMPLSERVICE_EXCEPTION!= null) {
            throw JAXTESTSERVICEIMPLSERVICE_EXCEPTION;
        }
        return JAXTESTSERVICEIMPLSERVICE_WSDL_LOCATION;
    }

}
